...
const propPosition = computed(() =>
  irPosition.value === "right" ? "bottom" : "right",
);
...
// ── Drag-to-resize ──
function onResizePalette(e) {
  resize(e, paletteW, "x", 160, 500, 1);
}

function onResizeRight(e) {
  const target =
    props.irOpen && irPosition.value === "right" ? irRightW : propRightW;
  resize(e, target, "x", 180, 700, -1);
}

function onResizeIrBottom(e) {
  resize(e, irBottomH, "y", 80, 600, -1);
}
function onResizePropBottom(e) {
  resize(e, propBottomH, "y", 80, 400, -1);
}
  ...
  <div class="editor-root">
    ...
    >
      ...
      <aside class="panel panel-left">
        ...
        <div
          class="drag-edge drag-edge--right"
          @pointerdown="onResizePalette"
        />
      </aside>
      ...
      <div class="center-column">
        ...
        <template v-if="irPosition === 'bottom'">
          <div class="ir-preview-wrapper" :class="{ open: irOpen }">
            <div
              v-if="irOpen"
              class="drag-edge drag-edge--top"
              @pointerdown="onResizeIrBottom"
            />
            <div
              class="ir-preview-header"
              @click="emit('update:irOpen', !irOpen)"
            >
              <span>IR Preview</span>
              <span class="ir-header-actions">
                <button
                  class="ir-pos-btn"
                  title="Move IR to right (Properties goes right)"
            ...
            <div
              v-show="irOpen"
              class="ir-preview-body"
              :style="{ height: irBottomH + 'px' }"
            >
              <IrPreview />
            </div>
          </div>
        ...
        <template v-if="propPosition === 'bottom'">
          <div class="prop-bottom-wrapper">
            <div
              class="drag-edge drag-edge--top"
              @pointerdown="onResizePropBottom"
            />
            <div class="prop-bottom-header">
              <span>Properties</span>
            </div>
            <div
              class="prop-bottom-body"
              :style="{ height: propBottomH + 'px' }"
            >
              <PropertyPanel />
            </div>
          </div>
      ...
      <aside class="panel panel-right">
        ...
        <template v-if="irOpen && irPosition === 'right'">
          <div
            class="ir-preview-header ir-preview-header--right"
            @click="emit('update:irOpen', !irOpen)"
          >
            <span>IR Preview</span>
            <span class="ir-header-actions">
              <button
                class="ir-pos-btn"
                title="Move IR to bottom, Properties to right"
                @click.stop="toggleIrPosition"
              >
                ⇤
              </button>
